...
(function () {
	...
	function bindCreateBtn() {
		...
		addBtn.addEventListener('click', () => {
			if (window.starlImportPlaylist && typeof window.starlImportPlaylist.openChooser === 'function') {
				window.starlImportPlaylist.openChooser();
			} else if (window.starlPlaylists) {
	...
	function setSearchImage(el, imageUrl) {
		...
		// defer the load until the row scrolls near the viewport (see media-cache.lazyImageEl)
	...
	function renderSearchContent(results, filter) {
		...
		if (!list) return;
		list.innerHTML = '';
		// build rows off-DOM and insert once - one reflow instead of one per result
	...
	// one playlist edit fans out into several 'starl-playlists-updated' events (direct
	// dispatch + account-state setState round-trips). Coalesce the burst into a single
	// reconcile on the next frame so the card list settles once instead of thrashing
	let playlistsRenderScheduled = false;
	function scheduleRenderPlaylists() {
		if (playlistsRenderScheduled) return;
		playlistsRenderScheduled = true;
		const run = () => {
			playlistsRenderScheduled = false;
			renderPlaylists();
		};
		if (typeof requestAnimationFrame === 'function') requestAnimationFrame(run);
		else setTimeout(run, 16);
	}
	window.addEventListener('starl-playlists-updated', scheduleRenderPlaylists);
