...
(function () {
	...
	function renderArtistDetail(artist) {
		...
		fetchArtistFromServer(artist.name, artist.channelId || null)
			.then((remote) => {
				...
				// for this artist - two artists can share a name (ex: "BEX" punk rock artist vs "BEX" Bollywood fucks)
				...
				// pick-up artists row
				if (window.starlStats && remote.photo_url)
					window.starlStats.recordArtistImage(remote.name || artist.name, remote.photo_url);
	...
	function openArtist(artist, push) {
		if (!overlayEl) init();
		if (!overlayEl) return;
		if (window.starlStats && artist)
			window.starlStats.recordArtistOpen({
				name: artist.name,
				channelId: artist.channelId,
				imageUrl: artist.imageUrl || artist.photo_url || artist.thumbnail || '',
			});
		if (push) {
	...
	function openLocalAlbum(album) {
		if (!overlayEl) init();
		if (!overlayEl) return;
		if (window.starlStats && album)
			window.starlStats.recordAlbumClick({name: album.name, artist: album.artist, imageUrl: album.imageUrl});
	...
	function openServerAlbum(item, push) {
		if (!overlayEl) init();
		if (!overlayEl) return;
		if (window.starlStats && item)
			window.starlStats.recordAlbumClick({
				id: item.id,
				title: item.title,
				artist: item.artist,
