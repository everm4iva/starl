...
(function () {
	...
	function setImageAsync(el, imageUrl, opts) {
		...
		// lazyImageEl defers the load until the row/card nears the viewport - covers
		// stream in on scroll instead of all firing at once when a list opens
		if (cache && typeof cache.lazyImageEl === 'function') {
			cache.lazyImageEl(el, imageUrl, opts);
		} else if (cache && typeof cache.setImageEl === 'function') {
	...
	function removeArtistFromLibrary(artist) {
		...
		if (trackCount > 0) {
			const keys = artist.tracks.map((t) => t.trackKey || t.sourceUrl || '').filter(Boolean);
			// batched into one write - removing tracks one at a time fired one network
			// PUT per track, and the responses could race and resurrect some of them
			if (hist && typeof hist.removeMany === 'function') hist.removeMany(keys);
			keys.forEach((key) => {
