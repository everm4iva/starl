...
document.addEventListener('resume', async () => {
	try {
		...
		// refresh if the token expired or is about to, so the resumed stream request doesn't fail with 401
		...
		// service while backgrounded, leaving a failing fallback notification
		...
		// basically: yap yap yap, NEEERDDDDDD
...
function updateNativePlayingState() {
	...
	const pb = window.starlPlaybackState;
	// mid auto-advance the element blips "paused" for a moment (old track torn down, next
	// one still loading). don't push that transient pause to the native session - dropping
	// the session to paused here is part of what lets the OS deny the next background play().
	// keep the last "playing" state; it settles for real once the next track starts.
	if (pb.isLoadingTrack && pb.shouldBePlaying && audio.paused) {
		return;
	}
