...
(function () {
	...
	function setCoverSlotImage(el, imageUrl) {
		...
		// defer the load until the card nears the viewport (see media-cache.lazyImageEl)
		if (cache && typeof cache.lazyImageEl === 'function') {
			cache.lazyImageEl(el, imageUrl, {variant: 'low'});
			return;
		}
	...
	// one shared listener keeps every rendered playlist card's play/pause button in sync
	// here cards register their updater and a single listener disconnected ones.
	const playlistCardUpdaters = new Set();

	function refreshPlaylistCards() {
		playlistCardUpdaters.forEach((entry) => {
			if (!entry.el.isConnected) {
				playlistCardUpdaters.delete(entry);
				return;
			}
			entry.update();
		});
	}
	window.addEventListener('starl-playback-changed', refreshPlaylistCards);
	...
	function createPlaylistItem(playlist, onOpen) {
		...
		item.className = 'item' + (playlist.locked ? ' locked' : '');
		...
		playBtn.addEventListener('click', (e) => {
			...
			if (window.starlInsidePlaylist) {
				window.starlInsidePlaylist.open({
					...
					canReorder: !playlist.locked,
					canEdit: !playlist.locked,
		...
		// register with the shared playback listener (automatically once detached)
		playlistCardUpdaters.add({el: item, update: updatePlayBtn});
	...
	function createPlaylistTile(playlist, onOpen) {
		...
		tile.className = 'item tile-item' + (playlist.locked ? ' locked' : '');
	...
	// everything a card actually renders, boiled down to one string. If a card's signature
	// is unchanged we reuse its DOM element as-is (keeping its already-loaded cover images),
	// so a burst of 'starl-playlists-updated' events no longer blinks the whole list.
	// play/pause state is deliberately left out - a live listener keeps that in sync
	function playlistSignature(playlist) {
		const cover = getCoverTracks(playlist, playlist.coverMode)
			.slice(0, 4)
			.map((t) => (t && t.imageUrl) || '')
			.join(',');
		return [
	...
	function renderTo(containerEl, mode, onOpen) {
		...
		// switching between list and tile modes changes the card markup entirely, so a full
		// wipe is the honest move there; within the same mode we reconcile in place
		if (containerEl.dataset.renderMode !== mode) {
			containerEl.innerHTML = '';
			containerEl.dataset.renderMode = mode;
		}

		// index the cards already on screen by playlist id
		const existing = new Map();
		Array.from(containerEl.children).forEach((child) => {
			const id = child.dataset && child.dataset.playlistId;
		...
		playlists.forEach((playlist) => {
			desired.add(playlist.id);
			const sig = playlistSignature(playlist);
			let el = existing.get(playlist.id);
			// a card whose contents changed (title, covers, count...) gets rebuilt; others stay
			if (el && el.dataset.sig !== sig) {
				if (cursor === el) cursor = el.nextSibling;
				el.remove();
				existing.delete(playlist.id);
				el = null;
			}
			if (!el) {
				el = isTiles ? createPlaylistTile(playlist, onOpen) : createPlaylistItem(playlist, onOpen);
				el.dataset.sig = sig;
			}
			if (el === cursor) {
				cursor = cursor.nextSibling; // already in place
			} else {
				containerEl.insertBefore(el, cursor); // reuse-in-place or move
			}
		});
		...
		bs.open({
			render(body) {
				...
				if (!playlists.length) {
					...
					playlists.forEach((p) => {
						const row = document.createElement('div');
						row.className = 'bsc-action' + (p.locked ? ' ipl-add-locked-row' : '');
						const label = document.createElement('span');
						label.textContent = p.title;
						row.appendChild(label);

						// locked (imported-as-immutable) playlists can't receive tracks - the data
						// layer rejects it anyway (updatePlaylist), so show them read-only here
						// instead of silently no-opping on tap
						if (p.locked) {
							const tag = document.createElement('span');
							tag.className = 'ipl-lock-tag';
							tag.textContent = 'Locked';
							row.appendChild(tag);
							row.addEventListener('click', () => {
								if (window.starlLayout && typeof window.starlLayout.showToast === 'function') {
									window.starlLayout.showToast('That playlist is locked (read-only)');
								}
							});
						} else {
							// right-side "+" glyph: the row text sits on the left, but the brain
							// aims for the center/right of a button. The plus gives a clear
							// right-hand target and signals "tapping this row adds the track"
							const plus = document.createElement('span');
							plus.className = 'bsc-add-plus';
							plus.textContent = '+';
	...
	// clone via the data layer, toast, then open the copy so the user lands on it
	function duplicateAndOpen(playlistId) {
		if (!window.starlPlaylists || typeof window.starlPlaylists.duplicatePlaylist !== 'function') return;
		const copy = window.starlPlaylists.duplicatePlaylist(playlistId);
		if (!copy) return;
		if (window.starlLayout && typeof window.starlLayout.showToast === 'function') {
			window.starlLayout.showToast('Playlist duplicated');
		}
		if (window.starlInsidePlaylist && typeof window.starlInsidePlaylist.open === 'function') {
			window.starlInsidePlaylist.open({
				type: 'playlist',
				id: copy.id,
				title: copy.title,
	...
	function openSettingsModal(playlistId) {
		...
		// locked (imported-as-immutable) playlists can't be renamed/reordered - the only
		// action is removing them from the library
		if (playlist.locked) {
			bs.open({
				render(body) {
					const heading = document.createElement('div');
					heading.className = 'bsc-settings-header';
					heading.textContent = 'Imported playlist';
					body.appendChild(heading);

					const note = document.createElement('div');
					note.className = 'bsc-action ipl-locked-note';
					note.textContent = 'This playlist is locked (read-only).';
					note.style.opacity = '0.6';
					body.appendChild(note);

					// exporting a locked playlist is harmless (read-only), so it's still offered.
					const exportBtn = document.createElement('div');
					exportBtn.className = 'bsc-action';
					exportBtn.textContent = 'Export to JSON';
					exportBtn.addEventListener('click', () => {
						bs.close();
						setTimeout(() => {
							if (window.starlPlaylistExport) window.starlPlaylistExport.exportPlaylist(playlistId);
						}, 50);
					});
					body.appendChild(exportBtn);

					// duplicating a locked (imported) playlist yields an unlocked, editable copy
					const dupBtn = document.createElement('div');
					dupBtn.className = 'bsc-action';
					dupBtn.textContent = 'Duplicate playlist';
					dupBtn.addEventListener('click', () => {
						bs.close();
						setTimeout(() => duplicateAndOpen(playlistId), 50);
					});
		...
		bs.open({
			render(body) {
				...
				updateBtn.className = 'bsc-action';
				updateBtn.textContent = 'Update via JSON';
				updateBtn.addEventListener('click', () => {
					bs.close();
					setTimeout(() => {
						if (window.starlPlaylistExport) window.starlPlaylistExport.updatePlaylistFromFile(playlistId);
					}, 50);
				});
				body.appendChild(updateBtn);

				const repairBtn = document.createElement('div');
				repairBtn.className = 'bsc-action';
				repairBtn.textContent = 'Repair playlist';
				repairBtn.addEventListener('click', () => {
					// repair opens its own progress sheet in this same slot - no close needed
					if (window.starlPlaylistRepair) window.starlPlaylistRepair.repair(playlistId);
				});
