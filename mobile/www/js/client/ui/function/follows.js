...
(function () {
	...
	function writeFollows(next) {
		...
		try {
			localStorage.setItem('starl_follows', JSON.stringify(next));
		} catch (e) {}
		try {
			window.dispatchEvent(new CustomEvent(UPDATE_EVENT, {detail: {follows: next}}));
		} catch (e) {}
	}
	...
	window.addEventListener('starl-account-state-updated', () => {
		try {
			window.dispatchEvent(new CustomEvent(UPDATE_EVENT, {detail: {follows: readFollows()}}));
		} catch (e) {}
	});
})();
