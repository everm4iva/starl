...
(function () {
	...
	// feature-brand colors that shouldn't be user-themeable (they belong to specific UIs)
	const EXCLUDED_COLOR_VARS = new Set(['--spotify-green', '--ipl-accent']);
	...
	function getColorVarNames() {
		...
		factory.rootDecls.forEach((value, key) => {
			...
			if (EXCLUDED_COLOR_VARS.has(key)) return; // feature-brand colors, hidden from the picker
