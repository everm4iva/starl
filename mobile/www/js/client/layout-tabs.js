...
function setLoadingState(isLoading) {
	...
	// spinner overlaid on the cover art (mini + maximized player) instead of the old sweep bar
	document.querySelectorAll('.mini-player-cover, .mp-cover').forEach((el) => {
		el.classList.toggle('cover-loading', loading);
	});
	if (bottomNav) {
		bottomNav.classList.toggle('is-loading', loading);
	}
}
