...
public class StarlIntent extends CordovaPlugin {
    ...
    // pull the first YouTube/Spotify URL out of a shared text blob.
    // apps typically share "title - https://youtu.be/ID" or just the URL...
    private String extractMusicUrl(String text) {
        if (text == null) return null;
        String trimmed = text.trim();
        // single bare URL
        if (trimmed.startsWith("http") && !trimmed.contains(" ") && isMusicUrl(trimmed)) {
            return trimmed;
        }
        // URL embedded in longer text
        for (String word : trimmed.split("\\s+")) {
            if (word.startsWith("http") && isMusicUrl(word)) {
                return word;
            }
        }
        return null;
    ...
    private boolean isMusicUrl(String url) {
        ...
        return url.contains("youtube.com") || url.contains("youtu.be") || url.contains("spotify.com");
    }
}
