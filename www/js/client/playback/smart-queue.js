...
(function () {
	...
	// recovery modes, discovered by the UI from this list
	const MODES = [
	...
	// retry tuning. defaults and the hard cap live here so nothing is hardcoded
	// at the call sites or in the markup.
	...
	async function skip(direction) {
		...
		// even if no other cached track exists - still "handle" it - just don't
		// move, rather than land the player on an unplayable track.
