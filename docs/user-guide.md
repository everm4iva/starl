...
Starl uses OAuth via **Google** or **Discord** - no passwords stored anywhere. Tap your preferred option on the welcome screen; the app will open a browser for you to complete the login, then return automatically.
...
- The **Library** tab holds your playlists, liked songs, and listening history.
- **Star** a track to add it to your favorites.
- Tap **⋮** on any track to open the context menu (add to playlist, view album/artist, etc.).

## Search

- Use the **Search** tab to find artists, albums, and songs.
- Recent searches are saved and shown when you open the tab.
...
- You can also easily delete your account from the **Account** tab at any time.

## Gestures

Starl is gesture-driven. The system back button is not used.
