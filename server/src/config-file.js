...
server:
  name: "MyStarlServer"        # the name people see on the status page and in the app, do not put space or special characters in it, just letters and numbers
  description: "just a cozy little music server"
  picture: ""                    # path or url to a picture, leave empty for the default one
  port: 6977                    # the api port, where the app talks to the server
  page_port: 6911               # the page port, where the little website and status page live
...
auth:
  mode: "pin"               # how people get in: none | pin | password | userpass
  pin: "2233"                        # pin mode only: the shared numeric code you pick, like "4823"
...
worker:
  port: 6918                     # the python resolver worker port, just internal plumbing
...
function ensureSecret(doc) {
	...
	try {
		...
		/* if we cant write it, the in-memory one still works for this run */
	}
}
